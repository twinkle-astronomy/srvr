...
mod tests {
    ...
    async fn saved_query_appears_in_template_list() {
        ...
        let template = crate::db::create_template(
            &format!("api_prom_tpl_{}", std::process::id()),
            "<svg/>",
        )
        .await
        .expect("create template");
        let name = format!("api_prom_q_{}", std::process::id());

        let response = router
            .clone()
            .oneshot(
                Request::post("/prometheus")
        ...
        let saved: crate::models::PrometheusQuery = serde_json::from_slice(&body).unwrap();
        assert!(saved.id.is_some(), "save should assign an id");

        let response = router
            .oneshot(
                Request::get(format!("/templates/{}/prometheus", template.id))
                    .header("cookie", &cookie)
                    .body(Body::empty())
                    .unwrap(),
            )
        ...
        let body = axum::body::to_bytes(response.into_body(), usize::MAX).await.unwrap();
        let queries: Vec<crate::models::PrometheusQuery> = serde_json::from_slice(&body).unwrap();
