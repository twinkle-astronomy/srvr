...
async fn device_from_headers(headers: &HeaderMap) -> Result<Device, Error> {
    ...
    let friendly_id = headers.get("Frendly-Id").and_then(|h| h.to_str().ok()).unwrap_or("Unknown");
    let model = headers.get("model").and_then(|h| h.to_str().ok());
    let battery_voltage = headers.get("Battery-Voltage").and_then(|h| h.to_str().ok()).and_then(|x| x.parse().ok());
    let fw_version = headers.get("FW-Version").and_then(|h| h.to_str().ok());
    let rssi = headers.get("RSSI").and_then(|h| h.to_str().ok());
    let device_height = headers.get("Height").and_then(|h| h.to_str().ok()).and_then(|x| x.parse().ok());
    let device_width = headers.get("Width").and_then(|h| h.to_str().ok()).and_then(|x| x.parse().ok());
