...
#[component]
fn FirmwareUpdatesToggle(device_id: i64, current_value: bool) -> Element {
    let store = use_context::<AppStore>();
    let mut checked = use_signal(move || current_value);
    let mut save_status = use_signal(|| None::<Result<(), String>>);
    ...
    rsx! {
        div { class: "mt-4 pt-4 border-t border-gray-100",
            h2 { class: "text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3", "Firmware Updates" }
            div { class: "flex items-center gap-3",
                label { class: "relative inline-flex items-center cursor-pointer",
                    input {
                        r#type: "checkbox",
                        ...
                        onchange: move |evt| {
                            ...
                            save_status.set(None);
                            spawn(async move {
                                match store.update_device_firmware_updates_enabled(device_id, val).await {
                                    Ok(()) => save_status.set(Some(Ok(()))),
                                    Err(e) => save_status.set(Some(Err(e.to_string()))),
                                }
                            });
