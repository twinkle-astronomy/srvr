...
- **Bound the boundaries, not every await.** An await only stalls if something it transitively waits on can stall, and that happens where control leaves the process. Put a timeout there — once, at the shared client or wrapper (e.g. `reqwest::Client::builder().timeout(...)`, the single `call_anthropic_api` chokepoint) — and let interior awaits inherit it. Don't add timeouts to in-process waits; a hang there is a bug a timeout would hide. At each boundary, also handle every non-success terminal state explicitly (cancelled, superseded, truncated — not just `Err`), give the user a way to cancel work that is bounded but slow, and add `tracing` at the seam so a stall identifies itself from a console read. (From the ai-template-generation retro: all three field failures were unbounded or silently-swallowed boundary outcomes.)
- **Surface fallback outcomes on user-visible flows.** When a feature degrades gracefully — a parser returns `None`, a lookup misses, an optional enrichment fails — the UI must say what happened. A silent fallback is indistinguishable from the code not running at all, both to the user and to whoever debugs their report. Best-effort is fine; invisible is not. (From the firmware-ota-updates retro: version auto-detection silently returned `None` on real firmware, and the resulting bug report was "I'm not convinced the code is running.")
- **Fixture real artifacts — don't re-synthesize them.** When a bug is reproduced from a real artifact (a firmware binary, a wire capture, a malformed upload), check the artifact's actual bytes in as the regression fixture. A hand-built synthetic fixture encodes the same assumptions that produced the bug. (From the firmware-ota-updates retro: the synthetic ESP image validated a documented "fixed offset" that the real binary disproved; the fix landed with the real header bytes as the fixture in both test tiers.)
- **A new setting isn't done until it's reachable *and* observable.** When a plan is silent on how a flag gets set, treat "settable from the UI it lives next to" as default scope rather than an opt-in extra — and then ask what the user *sees* after they flip it. A control nothing can reach, or one whose effect is invisible, is an unfinished feature, not a smaller one. (From the 2-bit-grayscale retro, which shipped the same omission twice: first a device flag with no toggle, then a toggle whose effect no preview reflected.)
- **A plan isn't self-consistent until its examples and its assertions describe the same artifact.** When a plan's code sample and its test step disagree about what gets produced, that's a design decision surfacing late, not a detail to smooth over — stop and settle it before implementing either half. (From the 2-bit-grayscale retro: the plan's encoder snippet could only emit an 8-bit PNG, while its test step asserted a bit depth of 2. Neither half was wrong alone; they just didn't add up to one artifact.)
- **Don't state behavior of existing code without reading it.** A plan exists to be checkable before implementation, so assertions in one carry the weight of verified fact; the same goes for a failure diagnosis. Read the code path and cite it (`file.rs:line`), reproduce or instrument before concluding, or explicitly label the claim an assumption to confirm. (From the 2-bit-grayscale retro: a plan asserted the AI page read its render device "at render time" — it snapshots once per send, and the wrong claim was concealing a race. Separately, a stale-bundle symptom was diagnosed as a test defect and presented as fact.)
- **Exercise a new interactive control, in the browser, before calling it done.** Rendering a control is not exercising it. Anything with async side effects needs a browser test that actually operates it. (From the 2-bit-grayscale retro: the new preview-device dropdown had no test that opened it, and that is exactly where an infinite reactive loop lived — one that crashed the tab within seconds of a real user touching it.)
- **Check *how* a test fails, not just that it does.** A test that fails by crashing the browser, timing out, or erroring in setup has told you nothing about whether its assertion works. Failing for the intended reason is the bar. Validate a regression test by reintroducing the bug and confirming the *assertion* is what fires. (From the 2-bit-grayscale retro: a loop test failed on the real bug only because that loop was violent enough to kill the tab; a slower one would have slipped past its assertion.)
- **When a frontend fix appears not to have taken, suspect the build first.** `dx build --platform web`, restart, hard-reload — before re-opening the diagnosis or rewriting tests. A stale WASM bundle is indistinguishable from a fix that didn't work, and `dist/` in this repo can be months old. The server logs `serving dashboard assets from …` at startup; check that directory's `.wasm` timestamp. (From the 2-bit-grayscale retro, where a correct, test-verified fix appeared to fail for exactly this reason.)
- **After adding or renaming a struct field, grep for hand-authored JSON fixtures of that struct** — `grep -rln '"sibling_field_name"'`. Serde deserialization of an inline test fixture breaks at runtime, not compile time, so a scoped test run stays green while an unrelated test starts failing. (From the 2-bit-grayscale retro: adding a non-optional `Device` field broke a `RenderContext` fixture in `api/templates.rs` with a 422, caught only by running the full suite.)
